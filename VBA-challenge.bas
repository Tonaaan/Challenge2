...
'Set header info
headers() = Array("Ticker", "Date", "Open", "High", "Low", "Close", "Volume", " ", "Ticker", "Yearly_Change", _
    "Percent_Change", "Stock_Volume", " ", " ", " ", "Ticker", "Value")
...
For Each MainWs In wb.Sheets
    ...
    .Rows(1).VerticalAlignment = x1Center
    End With
Next MainWs
    ...
    'Set initial variables for calculations
        ...
        'Loop from the beginning of the main worksheet(Row2) till its the last row of worksheet 7
        For i = 2 To Lastrow
            ...
            If MainWs.Cells(i + 1, 1).Value <> MainWs.Cells(i, 1).Value Then
                ...
                ElseIf (Yearly_PriceC_Change <= 0) Then
                
                    MainWs.Range("J" & Summary_Table_Row).Interior.ColorIndex = 3
                End If
                
                'Print the yearly price change as a percent in the Summary Tabvle, Column K
